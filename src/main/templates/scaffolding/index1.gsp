...
<head>
    ...
    <script type="text/javascript">
        ...
        \$(function () {
            ...
            orderTable = new EasyUIExt(\$("#mainGrid"), "<g:createLink base=".." controller="goodsUnit" action="list"  />");
            orderTable.singleSelect = true;//是否单选
            orderTable.window = \$("#editWindow");//dialog
            orderTable.form = \$("#editForm");//表单
...
//            }
        ...
        function edit(id) {
            orderTable.mainEdit("<g:createLink base=".." controller="${propertyName}" action="edit"  />", "${tableComment}-修改", id);
            orderTable.formAction = "<g:createLink base=".." controller="${propertyName}" action="update"  />";
        }
        function del() {
            orderTable.mainDel("<g:createLink base=".." controller="${propertyName}" action="del"  />");
        }
        function doSearch() {
            \$("#mainGrid").datagrid({
                    queryParams: {
...
<div class="rel clearfix function-btn">
    ...
    <p opId="103_236" class="search search-width abs">
